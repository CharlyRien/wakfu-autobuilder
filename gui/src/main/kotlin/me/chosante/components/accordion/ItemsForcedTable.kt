...
class ItemsForcedTable : TableView<ItemForced>() {
    ...
    init {
        val columnSelection = TableColumn<ItemForced, Boolean>().apply {
        ...
        val columnItemName = TableColumn<ItemForced, String>(I18n.valueOf(I18nKey.ITEM_FORCED_TABLE_COLUMN_ITEM_NAME_TITLE))
            .apply {
                cellValueFactory = Callback { SimpleStringProperty(it.value.name) }
                cellFactory = TextFieldTableCell.forTableColumn()
            }
        val columnItemLevel = TableColumn<ItemForced, String>(I18n.valueOf(I18nKey.ITEM_FORCED_TABLE_COLUMN_ITEM_LEVEL_TITLE))
            .apply {
                cellValueFactory = Callback { SimpleStringProperty(it.value.level.toString()) }
                cellFactory = TextFieldTableCell.forTableColumn()
            }
        val columnItemRarity = TableColumn<ItemForced, String>(I18n.valueOf(I18nKey.ITEM_FORCED_TABLE_COLUMN_ITEM_RARITY_TITLE)).apply {
...
data class ItemForced(val name: String, val level: Int, val rarity: Rarity, val status: SimpleBooleanProperty)
