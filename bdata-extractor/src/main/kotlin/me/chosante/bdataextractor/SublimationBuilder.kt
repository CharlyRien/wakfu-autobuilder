...
// decoded (see [decodePerStatStepRamp] → [SublimationEffect.PerStatStep]). Every other 913 (conversions / best-element) stays
// forced-only.
...
fun buildSublimations(
    ...
    fun decodePerStatStepRamp(
        ...
        subtreeSet: Set<Int>,
        maxTier: Int,
    ): Pair<SublimationEffect.PerStatStep, Set<Int>>? {
        ...
        return SublimationEffect.PerStatStep(source, threshold, perStep, cap, target) to (setOf(rampId) + capChildren)
    }

    // First-party decode of a "convert 100% of <source> into <target>" 913 sub (Unraveling: crit mastery →
    // elemental mastery, gated by its CRIT_AT_LEAST condition), reusing the model's dormant [SublimationEffect.Conversion]
    ...
    fun decodeConversion913(
        subtree: List<Int>,
        subtreeSet: Set<Int>,
    ): Pair<SublimationEffect.Conversion, Set<Int>>? {
        ...
        return SublimationEffect.Conversion(from = source, to = target, percent = 100) to (setOf(convId) + children)
    }

    // First-party decode of the "best element" concentration sub (Elemental Concentration: +20% Damage Inflicted,
    // −30% mastery on the three weakest elements) → a [SublimationEffect.BestElementConcentration] (see that type for the sound
    ...
    fun decodeBestElementConcentration(
        ...
        subtreeSet: Set<Int>,
        maxTier: Int,
    ): Pair<SublimationEffect.BestElementConcentration, Set<Int>>? {
    ...
    return meta.sortedBy { it.stateId }.map { m ->
        ...
        val maxStackLevel = (state?.get("max_level") as? Int)?.coerceAtLeast(1) ?: 1
        ...
        if (timingFilter != null && (timingFilter.isEmpty() || m.stateId in timingFilter)) {
            ...
            if (subtree.isEmpty()) {
                println("subTiming state=${m.stateId} name=\"$nm\" maxStackLevel=$maxStackLevel (no decoded effects; stateRowPresent=${state != null})")
            } else {
        ...
        Sublimation(
            ...
            condition = condition,
            effects = merged + listOfNotNull(perStatStep, conversion, bestElementConcentration),
...
internal fun synthesizeRawText(
    ...
    effects.filterIsInstance<SublimationEffect.StatEffect>().forEach { parts.add(effectText(it)) }
...
private fun bestElementConcentrationText(b: SublimationEffect.BestElementConcentration): String =
...
private fun conversionText(c: SublimationEffect.Conversion): String {
...
private fun perStatStepText(s: SublimationEffect.PerStatStep): String {
...
private fun effectText(e: SublimationEffect.StatEffect): String {
    val label =
